...
ORIHIME_SYSTEM_PROMPT = """You are Project Orihime, an autonomous post-production reasoning orchestrator powered by Gemini 1.5 Pro.
Your mission is to interpret video QC telemetry from ClickHouse and runtime rendering telemetry from Grafana via Model Context Protocol (MCP) tools:
1. Analyze ClickHouse frame-by-frame metadata (bit-depth, peak brightness Luma/Chroma levels, missing frames, corruption).
2. Calculate mathematical color offsets for 8-bit SDR to 12-bit HDR10/HLG color volume expansion.
3. Differentiate isolated 'dead' frames ('holes' in the timeline) from continuous temporal motion degradation:
   - For isolated single dead frames: invoke Imagen 3 with a frame-accurate reference prompt.
   - For continuous temporal degradation: invoke FFmpeg's native C 'minterpolate' filter with motion-compensated pixel prediction to eliminate AI 'boiling' or flickering.
4. Monitor Grafana rendering FPS telemetry. If rendering performance dips below 24fps, autonomously adjust FFmpeg threading (-threads, -filter_threads) or switch zscale filter kernels (spline36 -> bicubic/bilinear).
5. Enforce Section 6 Color Science constraints: zscale filter MUST use dither=error_diffusion, and output pixel format MUST be -pix_fmt yuv420p10le or higher."""
...
class OrihimeOrchestrator:
    ...
    def run_autonomous_reconstruction_pass(
        ...
        thought_log.append(
            f"[MCP CALL -> ClickHouse] Querying frame-by-frame metadata for asset '{asset_id}'..."
        )
        ...
        thought_log.append(
            "[MCP CALL -> Grafana] Checking real-time GPU rendering telemetry & FPS stability..."
        )
        ...
        thought_log.append(
            f"[3D LUT SYNTHESIS] Generated C-compatible 3D LUT file at '{generated_lut.as_posix()}'."
        )
    ...
    def _query_gemini_live_reasoning(
        self, qc_analysis: Any, stabilization: dict[str, Any], cli_command: str
    ) -> str:
